# sonic cli show command python code
# /usr/local/lib/python3.9/dist-packages/show/main.py
...
def route_map(route_map_name, verbose):
    """show route-map"""
    cmd = [constants.RVTYSH_COMMAND, '-c', 'show route-map']
...
def prefix_list(prefix_list_name, verbose):
    """show ip prefix-list"""
    cmd = [constants.RVTYSH_COMMAND, '-c', 'show ip prefix-list']
...
def protocol(verbose):
    ...
    cmd = [constants.RVTYSH_COMMAND, '-c', "show ip protocol"]
    run_command(cmd, display_cmd=verbose)
...
def prefix_list(prefix_list_name, verbose):
    """show ip prefix-list"""
    cmd = [constants.RVTYSH_COMMAND, '-c', 'show ipv6 prefix-list']
    if prefix_list_name is not None:
        cmd[-1] += ' {}'.format(prefix_list_name)
    run_command(cmd, display_cmd=verbose)
...
def protocol(verbose):
    ...
    cmd = [constants.RVTYSH_COMMAND, '-c', "show ipv6 protocol"]
    run_command(cmd, display_cmd=verbose)
...
def logging(process, lines, follow, verbose):
    ...
    if os.path.exists("/var/log.tmpfs"):
        ...
        log_path = "/var/log"
    if follow:
        cmd = ['tail', '-F', '{}/syslog'.format(log_path)]
        run_command(cmd, display_cmd=verbose)
    else:
        if os.path.isfile("{}/syslog.1".format(log_path)):
            cmd = "cat {}/syslog.1 {}/syslog".format(log_path, log_path)
        else:
            cmd = "cat {}/syslog".format(log_path)
